...
namespace PackTracker.Infrastructure.Services
{
    ...
    public class SettingsService : ISettingsService
    {
        ...
        public SettingsService(ILogger<SettingsService> logger, string defaultConfigPath)
        {
            _logger = logger;
            ...
            if (exeFolder != null) _defaultConfigPath = Path.Combine(exeFolder, "appsettings.json");
            EnsureUserConfigExists();

            _settings = LoadSettings();

            _watcher = new FileSystemWatcher(userFolder, "settings.json") { NotifyFilter = NotifyFilters.LastWrite };
        ...
        private void EnsureUserConfigExists()
        {
            if (!File.Exists(_userConfigPath))
            {
                try
                {
                    // Copy from default config if available, else create blank
                    ...
                    else
                    {
                        ...
                        {
                            ["AppSettings"] = JsonSerializer.SerializeToNode(new AppSettings(),
                                new JsonSerializerOptions { WriteIndented = true })
                        };
                        File.WriteAllText(_userConfigPath,
